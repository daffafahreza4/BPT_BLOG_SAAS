<x-blog-layout title="{{ $post_title }}">
    ...
    <div class="container mx-auto flex flex-wrap py-6">
            ...
            */
            ...
            const toInject = {
                    ...
                    addChild(child){
                        ...
                        // working on define
                        if(child.autoDefine)child.define();
                    ...
                    define(){
                        ...
                        this.findall('div').forEach((div)=>{
                            if(!div.classList.contains('dontdefine')){
                                if(div.id.length !== 0)
                                    this[div.id] = div;
                            }
                        })
        ...
        <script type="text/javascript">
            const div = find('article div');
            const content_level = {{ $post->membership_id }};
            let is_login,user_membership;
            @auth
                is_login = true;
                user_membership = {{auth()->user()->membership_id}};
            @endauth
            const forceMemberingSystem = ()=>{
                div.style.position = 'relative';
                div.style.height = '400px';
                div.style.overflow = 'hidden';
                div.addChild(makeElement('div',{
                    style:`
                        position:absolute;
                        left:0;
                        top:0;
                    ...
                    innerHTML:`
                        <div style=display:flex;flex-direction:column;gap:20px;>
                            <div style=font-size:24px;>Konten Terkunci!</div>
                            <div>Upgrade Membership Untuk Membaca Konten Ini!</div>
                            <a href=/memberships>
                                <div style="
                                    padding: 10px;
                                    background: black;
            ...
            if(content_level !== 1){
                ...
                if(!is_login || content_level !== user_membership){
                    forceMemberingSystem();
                }
            }
